# DozenOS ISO build AGENTS.md file

## Project purpose

Top-level DozenOS image builder. Orchestrates the multi-step build that produces
a hybrid live ISO from Debian packages and DozenOS-specific `.deb`s. The official
starting point for anyone building a DozenOS image from source. The ISO is
bootable on both BIOS and UEFI systems.

## Tech stack

- Python entry-point for ISO image build is (`build-dozenos-image`) and some
  `Makefile` wrappers.
...
- Configuration: TOML used as markup language which supports merging multiple
  ...
  * `data/defaults.toml`
- TOML files are combined by `build-dozenos-image` into a single Python dict and
...
./build-dozenos-image generic              # equivalent direct call
```

- Must run inside the build container. Container `Dockerfile` located in `docker`
  folder. Use `docker build -t dozenos/dozenos-build docker` from top-level dir.
- Git submodules are not in use. Prebuilt binary packages are pulled from
  `https://packages.dozenos.net/repositories/<train>` at build time.
...
- `build-dozenos-image` - Python entry-point.
- `Makefile` - flavor dispatcher (`make <flavor>` → `./build-dozenos-image <flavor>`).
- `data/defaults.toml` - Holds cross-flavor defaults like Linux Kernel version,
  URL to DozenOS Debian package repository, release branch/train or bootloaders
...
- All listed packages can be found in the GitHub `dozenos` organisation
- Consumes pre-built `*.deb` packages. The most important ones are:
  * `dozenos-1x` CLI representation and all configure/op-mode scripts
  * `dozenos-cloud-init` our Cloud-init handler
  * `dozenos-http-api-tools` HTTP API RESTful and GraphQL
...
- Smoketests inside the QEMU harness exercise `dozenos-1x`'s `smoketest/` suite.

## PR instructions

- Commit/PR title must follow: `component: T1234: description`. Phorge IDs at
  https://dozenos.dev. Enforced by `check-pr-message.yml` reusable workflow.
...
- Linting: unused-imports (Pylint) and J2 lint (note: workflow file is named
  ...
  `dozenos/.github@production`.
- PR conflicts are flagged automatically via `check-pr-conflicts.yml` (reusable
  `check-pr-merge-conflict.yml` from `dozenos/.github@production`).
...
- Reusable workflow `trigger_rebuild_packages.yml` fires REST
  `workflow_dispatch` into `$REMOTE_OWNER/dozenos-build-packages` (REMOTE_OWNER =
  the private side). The dispatcher runs as `dozenosbot`.
