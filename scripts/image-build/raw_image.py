...
import dozenos.utils.process

import dozenos.template

dozenos.template.DEFAULT_TEMPLATE_DIR = os.path.join(os.getcwd(), 'build/dozenos-1x/data/templates')
...
def mount_image(con):
    ...
    dozenos.system.disk.filesystem_create(efi, 'efi')
    dozenos.system.disk.filesystem_create(root, 'ext4')
...
def install_image(con, version):
    ...
    dozenos_dir = os.path.join(con.raw_dir, f'boot/{version}/')
    mkdir(dozenos_dir)
    mkdir(os.path.join(dozenos_dir, 'work/work'))
    mkdir(os.path.join(dozenos_dir, 'rw'))

    shutil.copy(f"{con.iso_dir}/{SQUASHFS_FILE}", f"{dozenos_dir}/{version}.squashfs")
...
def setup_grub_configuration(build_config, root_dir) -> None:
    ...
    # create new files
    dozenos.template.render(grub_cfg_main, grub.TMPL_GRUB_MAIN, {})
    ...
    dozenos.template.render(grub_cfg_menu, grub.TMPL_GRUB_MENU, {})
    dozenos.template.render(grub_cfg_options, grub.TMPL_GRUB_OPTS, {})
...
def create_raw_image(build_config, iso_file, work_dir):
    from dozenos.system.disk import parttable_create
    ...
    with BuildContext(iso_file, work_dir, debug=True) as con:
        ...
        raw_file = f"dozenos-{version}-{build_config['build_flavor']}-{build_config['architecture']}.raw"
