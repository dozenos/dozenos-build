...
if [ "${ARCH}" = "arm64" ]; then
    KCONFIG_MERGE_FRAGMENTS+=("${CWD}/config/arm64/dozenos_defconfig")
elif [ "${ARCH}" = "amd64" ]; then
    KCONFIG_MERGE_FRAGMENTS+=("${CWD}/config/x86/dozenos_defconfig")
else
...
# cause `make dozenos_defconfig` (and the rest of the kernel build) to write the
...
# DozenOS requires some small Kernel Patches - apply them here
...
sed -i -e "s/CN =.*/CN=DozenOS Networks build time autogenerated Kernel key/" certs/default_x509.genkey
