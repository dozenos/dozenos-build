...
DOZENOS_FIRMWARE_NAME="dozenos-linux-firmware"
...
DOZENOS_FIRMWARE_DIR="${DOZENOS_FIRMWARE_NAME}_${GIT_COMMIT}-0_${BUILD_ARCH}"
if [ -d ${DOZENOS_FIRMWARE_DIR} ]; then
    # remove Debian package folder and deb file from previous runs
    rm -rf ${DOZENOS_FIRMWARE_DIR}*
fi
mkdir -p ${DOZENOS_FIRMWARE_DIR}
...
# assembly folder for the dozenos-firmware package
...
for FILE_PATTERN in ${FW_FILES}; do
    find "${LINUX_FIRMWARE_BUILD_DIR}" -path "*/${FILE_PATTERN}" -print0 | while IFS= read -r -d $'\0' FILE; do
        ...
        TARGET_DIR="${DOZENOS_FIRMWARE_DIR}/lib/firmware/$(dirname "${TARGET}")"
        ...
        if [ -h "${FILE}" ]; then
		    ...
		    # If file links to other folder which this script not cover. Create folder and copy together.
                    if [ -L "${LINUX_FIRMWARE_BUILD_DIR}/${TARGET}" ]; then
                        ...
                        REALPATH_TARGET_DIR="${DOZENOS_FIRMWARE_DIR}/lib/firmware/$(dirname "${REALPATH_TARGET}")"
...
rm -f ${DOZENOS_FIRMWARE_NAME}_*.deb

PACKAGE_VERSION=$(debian_version "${GIT_COMMIT}")

cd ${DOZENOS_FIRMWARE_DIR}

debmake -n -y -p ${DOZENOS_FIRMWARE_NAME} -u ${PACKAGE_VERSION} \
    -e maintainers@dozenos.local -f "DozenOS Package Maintainers"

cat << EOF > debian/control
Source: ${DOZENOS_FIRMWARE_NAME}
Section: kernel
Priority: optional
Maintainer: DozenOS Package Maintainers <maintainers@dozenos.local>
Build-Depends: debhelper-compat (= 13)
Standards-Version: 4.5.1
Rules-Requires-Root: no

Package: ${DOZENOS_FIRMWARE_NAME}
Architecture: any
Depends: \${misc:Depends}
Description: Binary firmware for various drivers in the Linux kernel
 Firmware blobs assembled from linux-firmware.git for the drivers built
 into the DozenOS kernel.
EOF

cat << EOF > debian/rules
#!/usr/bin/make -f
PACKAGE_BUILD_DIR := debian/${DOZENOS_FIRMWARE_NAME}
