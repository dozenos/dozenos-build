#!/bin/sh
# DozenOS additions to Debian's live-boot.
#
...
#
# It exists so that DozenOS does not have to fork the live-boot source package.
# Everything below is either a translation of DozenOS-specific kernel command
...
# hooks/live/08-live-boot-dozenos.chroot build hook fails the ISO build if they
# have drifted.
#
...
# ---------------------------------------------------------------------------
# DozenOS kernel command line parameters
# ---------------------------------------------------------------------------
# dozenos-union=<path> (and its pre-1.2 alias vyatta-union=) is shorthand for the
# set of stock live-boot parameters that describe a DozenOS image installed to
# disk. It is still emitted by dozenos-1x (python/dozenos/system/grub.py,
# BOOT_OPTS_STEM) and by grub entries written by older releases, so it has to
# be understood indefinitely.
for _PARAMETER in $(cat /proc/cmdline)
do
	case "${_PARAMETER}" in
		dozenos-union=*|vyatta-union=*)
...
# DozenOS puts the boot medium and the persistence filesystem on the *same*
...
# parent directory on top of one of its own sub-directories. DozenOS avoids that by
...
#
# Debian live-boot 1:20151213, the base of the old DozenOS fork, did not blacklist
...
# makes stock live-boot incompatible with the DozenOS on-disk layout.
#
...
mount_persistence_media ()
{
	...
	# DozenOS: never mount a member of an assembled MD RAID directly - doing so
	...
	# DozenOS: flat backing directory, without the per-device sub-directory that
	# upstream uses. DozenOS boots a single persistence partition and addresses
...
get_custom_mounts ()
{
	...
	do
		...
		do
			...
			# DozenOS: resolve source= relative to persistence-path=, so that
